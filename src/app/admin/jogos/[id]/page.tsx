  ...
  return (
    ...
    <section className="space-y-6">
      ...
      <div className="overflow-hidden rounded-[1.75rem] bg-white/85 shadow-card">
        <div className="hidden grid-cols-[1.3fr_1fr_0.8fr_1fr] gap-3 border-b border-ink/10 p-4 text-sm font-bold text-leaf md:grid">
          <span>Nome</span>
          <span>Telefone</span>
          <span>Palpite</span>
          <span>Enviado em</span>
        </div>
        {match.bets.length ? match.bets.map((bet) => (
          <div key={bet.id} className="grid gap-2 border-b border-ink/10 p-4 text-sm last:border-b-0 md:grid-cols-[1.3fr_1fr_0.8fr_1fr] md:gap-3">
